...
%%%----检步
for i=1:mm
Acc(i)=sqrt(accer(i,2)*accer(i,2)+accer(i,3)*accer(i,3)+accer(i,4)*accer(i,4));
end
%%%平滑
a=3;   %平滑阶数
P=zeros(mm,1); 
...
for  i=12:mm
   Acc_But(i,1)=b(1,11)*P(i-11,1)-a(1,2)*P(i-1,1)-a(1,3)*P(i-2,1)-a(1,4)*P(i-3,1)-a(1,5)*P(i-4,1)-a(1,6)*P(i-5,1)-a(1,7)*P(i-6,1)-a(1,8)*P(i-7,1)-a(1,9)*P(i-8,1)-a(1,10)*P(i-9,1)-a(1,11)*P(i-10,1);
   end
%%%
[Count,StepPoint,~] = Jianbu(P,mm,gyros(:,1));%检步
%%%
...
%     grid on;
    ...
    for j=1:Count
        for i=1:mm
%             est=est+1;
%             temporary_accer(est,:)=exRot_accer(i,:);
%             temporary_filit_accer(est,:)=But_accer(i,:);
            if accer(i,1)==StepPoint(j+1)         % 航向角为人体航向/弧度
                [COEFF, score,latent,t2]=princomp(But_accer(est:i,:));
                [y(:,j)]= COEFF(:,1);%第一列为第一特征向量
                roll_1(j)=rad2deg(roll(i));
                a=[y(1,j),y(2,j)];
                yaw_PCA(j)=acos(dot(a,b)/(norm(a)*norm(b)))*180/pi;
                [COEF, scor,laten,t1]=princomp(exRot_accer(est:i,:));
                [yy(:,j)]= COEF(:,1);
                c=[yy(1,j),yy(2,j)];
                ya_PCA(j)=acos(dot(c,b)/(norm(c)*norm(b)))*180/pi;
                est=i+1;
%                 clear  temporary_accer  temporary_filit_accer;
                continue;
            end
        end
    end
    ...
    am=1;
    for i=3:j-1
        if i>am&&((abs(roll_1(i)-roll_1(i-1))>50&&abs(roll_1(i)-roll_1(i-1))<330)||(roll_1(i-1)-roll_1(i-2)>30&&roll_1(i)-roll_1(i-1)>30)||(roll_1(i-1)-roll_1(i-2)<-30&&roll_1(i)-roll_1(i-1)<-30))
            offset=offset+1;
            am=i+1;
            %         am(offset+1)=i+1;
...
% date=(abs(yaw_PCA(i)-yaw_PCA(i-1))+abs(yaw_PCA(i)-yaw_PCA(i-2))+abs(yaw_PCA(i)-yaw_PCA(i-3))+abs(yaw_PCA(i)-yaw_PCA(i-4))+abs(yaw_PCA(i)-yaw_PCA(i-5)))/5;
    ...
    figure(fist1)
    plot(1:Count,yaw_PCA_iHDE,'r','Linewidth',2);
    hold on
    plot(1:Count,yaw_PCA,'g','Linewidth',2);
    plot(1:Count,ya_PCA,'b','Linewidth',2);
    legend({'\bfiHDE+滤波+PCA','\bf滤波+PCA','\bfPCA'},'FontSize',12,'FontName','Times?New?Roman');
    title('\bf航向对比','FontSize',14)
...
%     end;
    ...
    axis equal;
    hold on
    plot(y2(1:j),x2(1:j),'-g*','MarkerSize',4,'linewidth',1)
    plot(y3(1:j),x3(1:j),'-b*','MarkerSize',4,'linewidth',1)
%      plot(zzy(1:101),zzi(1:101),'k','MarkerSize',4,'linewidth',1)
    plot(zzi(1:209),zzy(1:209),'k','MarkerSize',4,'linewidth',1)
    legend({'\bfiHDE+滤波+PCA','\bf滤波+PCA','\bfPCA','\bf参考路径'},'FontSize',12,'FontName','Times?New?Roman');
    ...
    %     subplot(3,1,1)
    %     axis equal;
    %     plot(gyros(:,1),rad2deg(yaw_2),'-b','MarkerSize',4,'linewidth',1)
    %     xlim([0 2.7])
    %     xlabel('时间/min','FontSize',12)
    %     ylabel('度/°','FontSize',12)
    %     title('\bf航向角','FontSize',14)
    %     zoom on;
    %     grid on;
    %
    %     subplot(3,1,2)
    %     axis equal;
    %     plot(gyros(:,1),rad2deg(pitch_2),'-g','MarkerSize',4,'linewidth',1)
    %     xlim([0 2.7])
    %     xlabel('时间/min','FontSize',12)
    %     ylabel('度/°','FontSize',12)
    %     title('\bf俯仰角','FontSize',14)
    %     zoom on;
    %     grid on;
    %
    %     subplot(3,1,3)
    %     axis equal;
    %     plot(gyros(:,1),rad2deg(roll_2),'k','MarkerSize',4,'linewidth',1)
    %     xlim([0 2.7])
    %     xlabel('时间/min','FontSize',12)
    %     ylabel('度/°','FontSize',12)
    %     title('\bf横滚角','FontSize',14)
    %     zoom on;
    %     grid on;
