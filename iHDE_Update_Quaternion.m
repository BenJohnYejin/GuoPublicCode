...
function [x2,y2,Count]=iHDE_Update_Quaternion()
clear all;
clc;
global h;
...
% h=1/(m/((zitai(m,1)-zitai(1,1))*60));
    [zitai(i,2),zitai(i,3),theta(i),cbn(i,1:3,1:3),p1(i),p2(i),p3(i),p4(i)]=Update_Quaternion2(p1(i-1),p2(i-1),p3(i-1),p4(i-1),tuoluoyi(i-1,2),tuoluoyi(i-1,3),tuoluoyi(i-1,4),tuoluoyi(i,2),tuoluoyi(i,3),tuoluoyi(i,4),h);
end;
% -----------------------------iHDE更新四元数------------------------------------%
dte=45;
for i=1:Count;
...
for i=1:Count
    for j=1:m
        if zitai(j,1)==StepPoint(i+1)
            thet(i)=theta(j)*180/pi;                                                        % 航向角为人体航向/弧度
            theta(i)=theta(j);
            zi(i,4)=zitai(j,4);
        end;
    end;
...
for i=1:Count
    ...
    [x2(i+1),y2(i+1)]=zuobiao(S,theta(i),x2(i),y2(i));
    [x3(i+1),y3(i+1)]=zuobiao(S,thet(i),x3(i),y3(i));
    [x4(i+1),y4(i+1)]=zuobiao(S,zi(i,4),x4(i),y4(i));
end;
%----------------------------理想行走路线--------------------%
zzi(1)=0;zzy(1)=0;
% for i=2:51
...
plot(y2(1:Count+1),x2(1:Count+1),'-g*','linewidth',2)
hold on;
plot(y3(1:Count+1),x3(1:Count+1),'-r*','linewidth',2)
xlabel('单位 /m');ylabel('单位 /m');
% plot(y4(1:Count+1),x4(1:Count+1),'-k*','linewidth',2)
plot(zzi(1:209),zzy(1:209),'k','linewidth',2)
plot(zzi(1),zzy(1),'bs','MarkerSize',8,'MarkerFaceColor',[0 0 1]); % marcar posicion inicial标记初始位置
